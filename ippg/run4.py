...
# ─────────────── Streaming frame buffer (shared across threads) ─────────────
...
class _StreamHandler(BaseHTTPRequestHandler):
    """Serves dash.html at '/' and an MJPEG stream at '/video_feed'."""
...
def _push_frame(bgr_frame: np.ndarray) -> None:
    """JPEG-encode a BGR frame and store it in the shared buffer."""
    global _latest_jpeg
    ok, buf = cv2.imencode(
        ".jpg", bgr_frame, [cv2.IMWRITE_JPEG_QUALITY, 80]
    )
    if ok:
        with _frame_lock:
...
class RPPGMonitor:
    ...
    # ---- Main loop --------------------------------------------------------
    def run(self):
        _start_stream_server(5050)          # ← launch browser stream server
